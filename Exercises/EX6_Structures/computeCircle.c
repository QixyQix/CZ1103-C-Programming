...
The function contain() returns 1 if  c1 contains c2, i.e. circle c2 is found inside circle c1.  Otherwise, the function returns 0.  
Circle c1 contains circle c2 when the radius of c1 is larger than or equal to the sum of the radius of c2 
and the distance between the centres of c1 and c2. Implement the function contain().
...
int intersect(struct circle c1, struct circle c2)
{
	...
	double distBetweenCentres = sqrt(pow((c1.x - c2.x),2) + pow((c1.y-c2.y),2));
	
	if(distBetweenCentres < (c1.radius + c2.radius)){
	    return 1;
	}else{
	    return 0;
	}
...
int contain(struct circle *c1, struct circle *c2)
{
	...
	if(c1->radius >= (distBetweenCentres + c2->radius)){
	    return 1;
	}else{
	    return 0;
	}
